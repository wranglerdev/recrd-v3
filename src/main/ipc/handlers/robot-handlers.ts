...
export function registerRobotHandlers(registry: IpcRegistry, useCases: RobotProjectUseCases): void {
  registry.handle("robot:scaffold", (request) => useCases.scaffold(request));
  registry.handle("robot:linkExisting", (request) => useCases.linkExisting(request));
}
