...
// `app:*` feature contract — process/app metadata (PRD §3). The template every
// other feature follows: a DTO block, a channel map, an `as const` name list, an
// API interface and a factory that maps API methods to channels via `invoke`.
...
export const APP_CHANNELS = defineChannelNames<AppChannels, ["app:getInfo"]>(["app:getInfo"]);
